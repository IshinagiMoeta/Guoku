...
public class ShapeIndicatorView extends View implements TabLayout.OnTabSelectedListener, ViewPager.OnPageChangeListener {
    ...
    private Path generatePath(int position, float positionOffset) {
        ...
        ViewGroup tabView = (ViewGroup) getTabViewByPosition(position);
        Drawable startDra;
...
//        tabView.setCompoundDrawables(null, startDra, null, null);
        float imgh = 0;
        if(img == null ){

            img = new ImageView(getContext());
            img.setImageResource(R.mipmap.tab_focus);
            tabView.addView(img);
            imgh = img.getX();
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
        ...
        generateDraw(position, positionOffset);
        invalidate();
    }

    private void generateDraw(int position, float positionOffset) {
        ViewGroup tabView = (ViewGroup) getTabViewByPosition(position);
    }
